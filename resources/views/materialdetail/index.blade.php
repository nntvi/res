...
<div class="table-agile-info">
        <div class="panel panel-default">
            ...
            <div class="col-sm-6 m-b-xs">
                <form enctype="multipart/form-data" role="form" action="{{route('material_detail.store')}}" method="POST" >
                    @csrf
                            ...
                            <form method="POST" action="{{route('material_detail.update',['id' => $materialDetail->id])}}">
                                ...
                                <td>
                                    <select class="form-control radius" name="type">
                                        <option value="{{$materialDetail->typeMaterial->id}}">{{$materialDetail->typeMaterial->name}}</option>
                                        @foreach ($types as $type)
                                            <option value="{{$type->id}}">{{$type->name}}</option>
                                        @endforeach
                                    </select>
